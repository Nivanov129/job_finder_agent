...
# Тонкий установщик собирает образ ИМЕННО из текущих файлов репозитория. Чтобы
# повторный запуск принёс свежие правки, сначала подтягиваем их из git (если это
# клон). Локальные изменения отслеживаемых файлов и не-git-каталоги не трогаем.
update_from_git() {
  command -v git >/dev/null 2>&1 \
    || { info "git не найден — пропускаю обновление кода (собираю из текущих файлов)."; return; }
  git -C "$REPO_DIR" rev-parse --is-inside-work-tree >/dev/null 2>&1 \
    || { info "Не git-репозиторий — пропускаю обновление кода (использую файлы как есть)."; return; }
  if [ -n "$(git -C "$REPO_DIR" status --porcelain --untracked-files=no)" ]; then
    warn "Есть локальные изменения отслеживаемых файлов — пропускаю git pull, чтобы их не потерять."
    return
  fi
  branch="$(git -C "$REPO_DIR" rev-parse --abbrev-ref HEAD 2>/dev/null || echo main)"
  info "Тяну последние обновления из git (ветка $branch)…"
  if retry 3 5 git -C "$REPO_DIR" pull --ff-only origin "$branch"; then
    info "Код обновлён до последней версии."
  else
    warn "git pull не удался (сеть/конфликт) — продолжаю на текущем коде."
  fi
}
update_from_git
...
info "Собираю образы заново (свежий код + кэш слоёв — обычно быстро)…"
